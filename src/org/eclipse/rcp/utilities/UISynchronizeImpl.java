...
/**
 * Implementation of the UISynchronize service for SWT.
 * <p>
 * Please note that this is a copy of org.eclipse.fx.ui.di.interopt.UISynchronizeImpl with a higher service ranking so
 * that we use this implementation instead of the default JavaFX one which relies on Platform.runLater instead of
 * Display.asyncExec.
 * </p>
 */
@Component(service = { ThreadSynchronize.class }, property = { "service.ranking:Integer=2" })
public class UISynchronizeImpl implements ThreadSynchronize {
	...
	public <V> V syncExec(final Callable<V> callable, V defaultValue) {
		if (Display.getDefault().getThread() == Thread.currentThread()) {
			...
			// future task
			RunnableFuture<V> task = new FutureTask<>(callable);
	...
	public Subscription scheduleExecution(long delay, Runnable runnable) {
		...
		display.asyncExec(() -> display.timerExec((int) delay, wrapper));
		...
		return new Subscription() {
			...
			public void dispose() {
				b.set(false);
				display.asyncExec(() -> display.timerExec(-1, runnable));
			}
		};
	}
	...
	public <T> CompletableFuture<T> scheduleExecution(long delay, Callable<T> runnable) {
		...
		Display display = Display.getDefault();
		display.asyncExec(() -> display.timerExec((int) delay, () -> {
			try {
	...
	@Override
	public <T> T block(BlockCondition<T> blockCondition) {
