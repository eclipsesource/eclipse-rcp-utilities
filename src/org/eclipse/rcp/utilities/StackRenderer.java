...
public class StackRenderer extends LazyStackRenderer implements IPreferenceChangeListener {
	 ...
	 *
	 */
	protected static final String THE_PART_KEY = "thePart"; //$NON-NLS-1$
	...
	private void addTopRight(CTabFolder ctf) {
		...
		ToolItem menuItem = new ToolItem(menuTB, SWT.PUSH);
		menuItem.setImage(getViewMenuImage());
		menuItem.setHotImage(null);
		menuItem.setToolTipText(SWTRenderersMessages.viewMenu);
	...
	public void adjustTopRight(final CTabFolder ctf) {
		...
		try {
			...
			boolean needsTB = part != null && part.getToolbar() != null
					...
					&& part.getToolbar().getChildren().size() > 0;
	...
	public void showAvailableItems(MElementContainer<?> stack, CTabFolder ctf) {
		...
		editorList.getShell().addListener(SWT.Deactivate, new Listener() {
			@Override
			public void handleEvent(org.eclipse.swt.widgets.Event event) {
				java.util.Optional<ThreadSynchronize> threadSynchronize = ServiceUtils.getService(ThreadSynchronize.class);
				if (threadSynchronize.isPresent()) {
					threadSynchronize.get().asyncExec(editorList::dispose);
				} else {
					Platform.runLater(editorList::dispose);
				}
			}
		});
	}
