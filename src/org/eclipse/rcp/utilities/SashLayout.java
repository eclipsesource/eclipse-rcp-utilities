...
public class SashLayout extends Layout {
	...
	private Supplier<Boolean> isWindows;

	public boolean layoutUpdateInProgress = false;

	public SashLayout(final Composite host, MUIElement root, Supplier<Boolean> isWindows) {
		this.root = root;
		this.host = host;
	...
	protected void layout(Composite composite, boolean flushCache) {
		...
		// It looks like as if on win32 the client-area is not fully
		// damaged (eg CTabFolder with round corners) so we need to force a redraw
		if (isWindows.get()) {
